...
inline ssize_t waveform_send_byte_data_packet(struct waveform_t* waveform, uint8_t* data, size_t data_size, enum waveform_packet_type type)
{
   vita_send_byte_data_packet(&waveform->vita, data, data_size, type);
}
